...
import path from 'path'; //Módulo para manejar rutas de los archivos
import { fileURLToPath } from 'url'; //Para obtener la ruta del archivo actual
import TareasDataSource from './tareasDataSource.mjs'; //Importamos la interfaz de persistencia
...
//Implementación concreta que extiende la interfaz TareasDataSoure
export default class TareaRepository extends TareasDataSource {
    ...
    //Implementación dek npetidi obtenerTodas()
    obtenerTodas() {
        try{
    ...
    guardar(tareas) {
        try {
            //Convertimos el array de tareas a una cadena JSON con identación de 2 espacios
